...
async function processPadImage(fileName) {
  ...
  // Placed a little above from the bottom (top = 724 instead of 758)
  // Deep purple crisp bold text on the clean lavender background strip
  const textSvg = Buffer.from(`
    <svg width="480" height="26" xmlns="http://www.w3.org/2000/svg">
      <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-weight="900" font-size="14" fill="#2E0E4D" letter-spacing="1.2">MANUFACTURED BY VIGOR LIFE CARE LLP</text>
    </svg>
  `);
  ...
  console.log(`Updated ${fileName} with company text placed a little above the bottom`);
}
...
async function processDiaperImage(fileName) {
  ...
  // Placed a little above the bottom (top = 615 instead of 658)
  const textSvg = Buffer.from(`
    <svg width="420" height="26" xmlns="http://www.w3.org/2000/svg">
      <text x="50%" y="50%" dominant-baseline="middle" text-anchor="middle" font-family="Arial, Helvetica, sans-serif" font-weight="900" font-size="13.5" fill="#003B73" letter-spacing="1.1">MANUFACTURED BY VIGOR LIFE CARE LLP</text>
    </svg>
  `);
